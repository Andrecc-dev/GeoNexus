...
import { updateTechTicketStatus } from '../services/dbService';
...
export default function TechMobileView({ technicians = [], tickets = [], onClose }) {
  ...
  const activeTicket = tickets.find(t => t.id === currentTech?.currentTicketId || (t.assignedTechId === currentTech?.id && t.status !== 'resolved'));
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 max-w-sm w-full space-y-4 shadow-2xl relative">
        ...
        <div className="text-center border-b border-slate-800 pb-3">
          ...
          <select 
            ...
            className="mt-2 w-full bg-slate-950 border border-slate-800 text-xs text-slate-200 rounded p-2 focus:border-sky-500 outline-none"
          >
            {technicians.map(t => (
        ...
        {activeTicket ? (
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-4 space-y-3">
            <div className="flex justify-between items-start">
              ...
              <span className="text-[10px] bg-sky-950 text-sky-400 px-2 py-0.5 rounded uppercase font-semibold">
                {activeTicket.status}
              </span>
            </div>

            <h4 className="text-sm font-bold text-white">{activeTicket.title}</h4>
            <p className="text-xs text-slate-400">📍 {activeTicket.location?.address}</p>

            <div className="flex gap-2 text-[10px] text-slate-300 bg-slate-900 p-2 rounded">
              <span className="flex items-center gap-1"><Clock className="w-3 h-3 text-amber-400" /> {activeTicket.estimatedRepairMinutes} min</span>
            ...
            <div className="space-y-2 pt-2">
              {activeTicket.status === 'assigned' && (
                <button 
                  ...
                  className="w-full bg-sky-600 hover:bg-sky-500 text-white text-xs font-bold py-2.5 rounded-lg flex items-center justify-center gap-2"
                >
              ...
              {activeTicket.status === 'traveling' && (
                <button 
                  ...
                  className="w-full bg-amber-600 hover:bg-amber-500 text-slate-950 text-xs font-bold py-2.5 rounded-lg flex items-center justify-center gap-2"
                >
              ...
              {activeTicket.status === 'in_progress' && (
                <button 
                  ...
                  className="w-full bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold py-2.5 rounded-lg flex items-center justify-center gap-2"
                >
          ...
          <div className="bg-slate-950 border border-slate-800 rounded-xl p-8 text-center space-y-2">
            ...
            <p className="text-[10px] text-slate-500">Status atual: Disponível para despacho.</p>
          </div>
        )}
      </div>
    </div>
